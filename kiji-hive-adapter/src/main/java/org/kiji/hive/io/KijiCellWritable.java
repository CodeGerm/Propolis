	...
	private static void writeData(DataOutput out, Object data, Schema schema) throws IOException {
		...
		case RECORD:
			...
			for (Schema.Field field : schema.getFields()) {
				...
				//LOG.warn("record type: "+(recordData.get(field.pos())).getClass().getName());
				if((recordData.get(field.pos())).getClass().getName().equals("org.apache.avro.util.Utf8"))
					writeData(out, (recordData.get(field.pos())).toString(), field.schema());
				else
		...
		case MAP:
			...
			for (Map.Entry<Object, Object> entry : mapData.entrySet()) {
				//LOG.warn("key type: "+(entry.getKey()).getClass().getName());
				//LOG.warn("value type: "+(entry.getValue()).getClass().getName());
				if((entry.getKey()).getClass().getName().equals("org.apache.avro.util.Utf8"))
					WritableUtils.writeString(out, (entry.getKey()).toString());
				else
					WritableUtils.writeString(out, entry.getKey().toString());
				if((entry.getValue()).getClass().getName().equals("org.apache.avro.util.Utf8"))
					writeData(out, (entry.getValue()).toString(),  schema.getValueType());
				else
